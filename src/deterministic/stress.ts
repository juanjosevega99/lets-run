 ...
 *
 *   1. HR-based TRIMP when average heart rate exists (Banister 1991, male weighting):
 *        TRIMP = minutes · r · 0.64 · e^(1.92·r),  r = (HRavg − HRrest)/(HRmax − HRrest)
 *   2. Pace-based stress for runs without HR (rTSS-shaped):
 *        stress = hours · (v / vThreshold)² · 100
 *      (threshold velocity from the athlete's reference VDOT at 86%)
 *   3. Flat duration rate for everything else (gym etc.) — crude, but §6: silent holes
...
/** Flat hourly rates for hierarchy level 3, by coarse sport bucket. Estimates, documented. */
...
export function isRunning(sportType: string): boolean {
  return /run/i.test(sportType) && !/virtual/i.test(sportType);
}
...
export function activityStress(
  ...
  const timeS = a.movingTimeS ?? a.elapsedTimeS;
  ...
  // 2 — pace-based, running with distance
